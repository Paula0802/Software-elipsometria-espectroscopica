...
async function executeOptimizationWithAlgorithm(algorithm, advancedConfig = {}) {
    try {
        ...
        // ⭐ CORREGIDO: respetar maxIterations del usuario con fallback a defaults
        const defaultIterations = algorithm === 'simplex' ? 500 : 300;
        const maxIterations = (advancedConfig.maxIterations && advancedConfig.maxIterations > 0)
            ? advancedConfig.maxIterations
            : defaultIterations;
 
        console.log(`🔁 Iteraciones: ${maxIterations} (usuario: ${advancedConfig.maxIterations || 'no especificado'}, default: ${defaultIterations})`);
 
        // ⭐ Config avanzada: combinar advancedConfig del modal de estrategia
        //    con advancedOptConfig del modal de configuración avanzada
        const mergedConfig = { ...advancedOptConfig, ...advancedConfig };
        ...
        const requestData = {
            ...
            max_iterations:          maxIterations,   // ⭐ CORREGIDO: ya no hardcodeado
            ...
            // ⭐ Config avanzada unificada (modal avanzado + modal estrategia)
            use_tikhonov_regularization: mergedConfig.use_tikhonov_regularization || false,
            lambda_reg:                  mergedConfig.lambda_reg                  || 1e-4,
 
            // Incertidumbres: solo si el usuario las configuró
            ...(mergedConfig.sigma_psi   != null && { sigma_psi:   mergedConfig.sigma_psi }),
            ...(mergedConfig.sigma_delta != null && { sigma_delta: mergedConfig.sigma_delta }),
        };
...
function handleConfirmAlgorithm() {
    ...
    const maxIterations     = parseInt(document.getElementById('maxIterations')?.value || '300'); // ⭐ NUEVO
