...
function updateAllPlots() {
    ...
    // ⭐⭐⭐ NUEVAS GRÁFICAS: Constantes Ópticas ⭐⭐⭐
    if (optimizationResults.optical_constants) {
        updateOpticalConstantsPlot(wavelengths, optimizationResults.optical_constants);
    }
    
    // ⭐⭐⭐ NUEVAS GRÁFICAS: T-R-A ⭐⭐⭐
    if (optimizationResults.tra_spectra) {
        updateTRAPlot(wavelengths, optimizationResults.tra_spectra);
    }
...
document.addEventListener('DOMContentLoaded', function() {
    // Toggle para constantes ópticas
    const toggleNK = document.getElementById('toggleTheoreticalNK');
    if (toggleNK) {
        toggleNK.addEventListener('change', function() {
            if (optimizationResults && optimizationResults.optical_constants) {
                updateOpticalConstantsPlot(
                    uploadedWavelengths, 
                    optimizationResults.optical_constants
                );
            }
    ...
    // Toggle para T-R-A
    const toggleTRA = document.getElementById('toggleTheoreticalTRA');
    if (toggleTRA) {
        toggleTRA.addEventListener('change', function() {
            if (optimizationResults && optimizationResults.tra_spectra) {
                updateTRAPlot(
                    uploadedWavelengths, 
                    optimizationResults.tra_spectra
                );
            }
        });
    }
...
// ==========================================
// SISTEMA DE PESTAÑAS DE VISUALIZACIÓN AVANZADA
// Agregar al final de app.js
// ==========================================

/**
 * Inicializa el sistema de pestañas de visualización
 * Llamar después de DOMContentLoaded
 */
function initVisualizationTabs() {
    console.log('🎨 Inicializando sistema de pestañas de visualización...');
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', function() {
            if (this.disabled) {
                console.log('⚠️ Pestaña deshabilitada:', this.dataset.tab);
                return;
            }
            
            const targetTab = this.dataset.tab;
            switchVisualizationPanel(targetTab);
        });
    });
    
    // Event listener para selector de capas (n,k)
    const layerSelect = document.getElementById('nk-layer-select');
    if (layerSelect) {
        layerSelect.addEventListener('change', function() {
            plotNKForSelectedLayer(this.value);
        });
    }
    
    // Event listeners para checkboxes de T-R-A
    ['showTransmittance', 'showReflectance', 'showAbsorbance'].forEach(id => {
        const checkbox = document.getElementById(id);
        if (checkbox) {
            checkbox.addEventListener('change', updateTRAPlotFromCheckboxes);
        }
    });
...
/**
 * Cambia entre paneles de visualización
 * @param {string} tabName - 'psi-delta', 'nk', o 'tra'
 */
function switchVisualizationPanel(tabName) {
    ...
    const activeTab = document.querySelector(`.viz-tab[data-tab="${tabName}"]`);
    if (activeTab) {
    ...
    if (activePanel) {
        ...
        // Renderizar gráfica según la pestaña
        setTimeout(() => {
            if (tabName === 'psi-delta') {
                renderPsiDeltaPanel();
            } else if (tabName === 'nk') {
...
/**
 * Muestra la sección de visualización avanzada y habilita pestañas según datos disponibles
 * @param {string} context - 'theoretical' o 'optimized'
 */
function showAdvancedVisualization(context = 'theoretical') {
    console.log(`📊 Mostrando visualización avanzada (contexto: ${context})`);
    
    const container = document.getElementById('advanced-visualization');
    if (!container) {
        console.error('❌ No se encontró #advanced-visualization');
        return;
    }
    ...
    // ========================================
    // PESTAÑA 1: Ψ y Δ (siempre habilitada)
    // ========================================
    const tabPsiDelta = document.querySelector('.viz-tab[data-tab="psi-delta"]');
    if (tabPsiDelta) {
        tabPsiDelta.disabled = false;
        tabPsiDelta.classList.remove('disabled');
    }
    
    // ========================================
    // PESTAÑA 2: n, k (habilitar si hay datos)
    // ========================================
    const hasOpticalConstants = checkOpticalConstantsAvailable(context);
    const tabNK = document.querySelector('.viz-tab[data-tab="nk"]');
    
    if (tabNK) {
        if (hasOpticalConstants) {
            tabNK.disabled = false;
            tabNK.classList.remove('disabled');
            tabNK.title = 'Ver constantes ópticas n, k';
            
            // Poblar selector de capas
            populateLayerSelector(context);
        } else {
            tabNK.disabled = true;
            tabNK.classList.add('disabled');
            tabNK.title = 'No hay datos de n, k disponibles';
        }
    }
    
    // ========================================
    // PESTAÑA 3: T-R-A (habilitar si hay datos)
    // ========================================
    const hasTRA = checkTRAAvailable(context);
    const tabTRA = document.querySelector('.viz-tab[data-tab="tra"]');
    
    if (tabTRA) {
        if (hasTRA) {
            tabTRA.disabled = false;
            tabTRA.classList.remove('disabled');
            tabTRA.title = 'Ver espectros de Transmitancia, Reflectancia y Absorbancia';
        } else {
            tabTRA.disabled = true;
            tabTRA.classList.add('disabled');
            tabTRA.title = 'No hay datos de T-R-A disponibles';
        }
    }
    
    // Activar primera pestaña por defecto
    switchVisualizationPanel('psi-delta');
    ...
    console.log(`✅ Visualización mostrada. n,k: ${hasOpticalConstants ? 'SÍ' : 'NO'}, T-R-A: ${hasTRA ? 'SÍ' : 'NO'}`);
}

/**
 * Verifica si hay datos de constantes ópticas disponibles
 */
function checkOpticalConstantsAvailable(context) {
    if (context === 'optimized') {
        return optimizationResults && 
               optimizationResults.optical_constants && 
               optimizationResults.optical_constants.layers &&
               optimizationResults.optical_constants.layers.length > 0;
    } else {
        return window.theoreticalOpticalConstants && 
               window.theoreticalOpticalConstants.layers &&
               window.theoreticalOpticalConstants.layers.length > 0;
    }
}

/**
 * Verifica si hay datos de T-R-A disponibles
 */
function checkTRAAvailable(context) {
    if (context === 'optimized') {
        return optimizationResults && 
               optimizationResults.tra_spectra &&
               optimizationResults.tra_spectra.T &&
               optimizationResults.tra_spectra.T.length > 0;
    } else {
        return window.theoreticalTRASpectra && 
               window.theoreticalTRASpectra.T &&
               window.theoreticalTRASpectra.T.length > 0;
    }
}

/**
 * Pobla el selector de capas para la gráfica de n,k
 */
function populateLayerSelector(context) {
    const selector = document.getElementById('nk-layer-select');
    if (!selector) return;
    
    // Limpiar opciones anteriores (excepto "Todas las capas")
    selector.innerHTML = '<option value="all">Todas las capas</option>';
    
    // Obtener datos de constantes ópticas
    let opticalConstants;
    if (context === 'optimized' && optimizationResults?.optical_constants) {
    ...
    if (!opticalConstants || !opticalConstants.layers) {
        console.warn('⚠️ No hay datos de capas para poblar selector');
        return;
    }
    
    // Agregar opción por cada capa
    opticalConstants.layers.forEach((layer, index) => {
        const option = document.createElement('option');
        option.value = index;
    ...
    console.log(`✅ Selector poblado con ${opticalConstants.layers.length} capas`);
}
...
/**
 * Renderiza el panel de Ψ y Δ
 */
function renderPsiDeltaPanel() {
    console.log('📈 Renderizando panel Ψ y Δ...');
    
    // Verificar que existan datos
    if (!uploadedWavelengths || uploadedWavelengths.length === 0) {
        document.getElementById('panel-psi-delta').innerHTML = `
            <div class="alert alert-warning">
                <i class="bi bi-exclamation-triangle me-2"></i>
                No hay datos experimentales cargados.
            </div>
        `;
        return;
    }
    ...
    const wavelengths = uploadedWavelengths;
    const psi_exp = uploadedPsi;
    const delta_exp = uploadedDelta;
    ...
    // Agregar curvas teóricas si existen
    if (theoreticalPsi && theoreticalPsi.length > 0) {
        traces.push({
            x: wavelengths,
            y: theoreticalPsi,
    ...
    // Agregar curvas optimizadas si existen
    if (optimizationResults) {
        if (optimizationResults.psi_theoretical) {
            traces.push({
                x: wavelengths,
                y: optimizationResults.psi_theoretical,
                mode: "lines",
        ...
        if (optimizationResults.delta_theoretical) {
            traces.push({
                x: wavelengths,
                y: optimizationResults.delta_theoretical,
                mode: "lines",
    ...
    const layout = {
        ...
        margin: { l: 60, r: 60, t: 40, b: 50 },
        title: {
            text: optimizationResults ? 'Ψ y Δ - Comparación Experimental vs Ajustado' : 'Ψ y Δ - Experimental vs Teórico',
            font: { size: 14 }
        },
        xaxis: { 
            title: "Longitud de onda (nm)",
    ...
    // Crear gráfica
    const plotDiv = document.getElementById('psi-delta-graph');
    if (plotDiv) {
    ...
    console.log('✅ Panel Ψ y Δ renderizado');
}

/**
 * Renderiza el panel de n, k
 */
function renderNKPanel() {
    console.log('📈 Renderizando panel n, k...');
    ...
    if (!opticalConstants || !opticalConstants.layers || opticalConstants.layers.length === 0) {
        const container = document.getElementById('panel-nk');
        if (container) {
            const graphN = document.getElementById('nk-graph-n');
            const graphK = document.getElementById('nk-graph-k');
            
            if (graphN) {
                graphN.innerHTML = `
                    <div class="alert alert-info">
                        <i class="bi bi-info-circle me-2"></i>
                        No hay datos de constantes ópticas disponibles.
                        <br><small>Calcula los valores teóricos primero.</small>
                    </div>
                `;
            }
            if (graphK) {
    ...
    plotNKForSelectedLayer(selectedValue);
}

/**
 * Plotea n y k para la capa seleccionada
 * @param {string} selectedValue - 'all' o índice de la capa
 */
function plotNKForSelectedLayer(selectedValue) {
    console.log(`📊 Ploteando n,k para: ${selectedValue}`);
    ...
    if (!opticalConstants || !opticalConstants.layers) {
        console.warn('⚠️ No hay datos de constantes ópticas');
        return;
    }
    ...
    const colors = [
        '#2E86C1', '#E74C3C', '#28a745', '#9C27B0', 
        '#FF5722', '#00BCD4', '#795548', '#607D8B'
    ];
    
    // ========================================
    // GRÁFICA DE n (índice de refracción)
    // ========================================
    const tracesN = [];
    
    layersToPlot.forEach((layer, i) => {
    ...
    const graphN = document.getElementById('nk-graph-n');
    if (graphN) {
        Plotly.newPlot(graphN, tracesN, layoutN, {
            displayModeBar: true,
            responsive: true
        });
    }
    ...
    // ========================================
    // GRÁFICA DE k (coeficiente de extinción)
    // ========================================
    const tracesK = [];
    ...
    const graphK = document.getElementById('nk-graph-k');
    if (graphK) {
        Plotly.newPlot(graphK, tracesK, layoutK, {
            displayModeBar: true,
            responsive: true
        });
    }
...
/**
 * Renderiza el panel de T-R-A
 */
function renderTRAPanel() {
    console.log('📈 Renderizando panel T-R-A...');
    ...
    if (!traData || !traData.T) {
        const container = document.getElementById('tra-graphs');
        if (container) {
            container.innerHTML = `
                <div class="alert alert-info">
                    <i class="bi bi-info-circle me-2"></i>
                    No hay datos de T-R-A disponibles.
                    <br><small>Calcula los valores teóricos primero.</small>
                </div>
            `;
        }
    ...
    updateTRAPlotFromCheckboxes();
}

/**
 * Actualiza la gráfica T-R-A según los checkboxes seleccionados
 */
function updateTRAPlotFromCheckboxes() {
    // Obtener estado de checkboxes
    const showT = document.getElementById('showTransmittance')?.checked ?? true;
    const showR = document.getElementById('showReflectance')?.checked ?? true;
    const showA = document.getElementById('showAbsorbance')?.checked ?? true;
    ...
    if (!traData) {
        console.warn('⚠️ No hay datos de T-R-A');
        return;
    }
    ...
    // Layout
    const layout = {
        title: { text: 'Espectros de Transmitancia, Reflectancia y Absorbancia', font: { size: 14 } },
        xaxis: { 
            title: 'Longitud de onda (nm)',
            showgrid: true,
            gridcolor: '#eee',
        ...
        // Agregar anotación de conservación de energía
        annotations: [{
            x: 0.02,
            y: 0.98,
            xref: 'paper',
    ...
    console.log('✅ Gráfica T-R-A actualizada');
}
...
/**
 * Descarga la gráfica de Ψ y Δ como PNG
 */
function downloadPsiDeltaGraphPNG() {
    const graphDiv = document.getElementById('psi-delta-graph');
    if (!graphDiv) {
    ...
    Plotly.downloadImage(graphDiv, {
        ...
        width: 1200,
        height: 700,
        filename: `psi_delta_${new Date().toISOString().slice(0,10)}`
    });
}

/**
 * Descarga los datos de Ψ y Δ como CSV
 */
function downloadPsiDeltaDataCSV() {
    if (!uploadedWavelengths || uploadedWavelengths.length === 0) {
        alert('No hay datos para descargar');
        return;
    }
    
    let csvContent = 'Wavelength_nm,Psi_exp,Delta_exp';
    
    // Agregar columnas según disponibilidad
    if (theoreticalPsi && theoreticalPsi.length > 0) {
        csvContent += ',Psi_teorico,Delta_teorico';
    }
    if (optimizationResults) {
        csvContent += ',Psi_optimizado,Delta_optimizado';
    }
    csvContent += '\n';
    
    for (let i = 0; i < uploadedWavelengths.length; i++) {
        let row = `${uploadedWavelengths[i]},${uploadedPsi[i]},${uploadedDelta[i]}`;
        
        if (theoreticalPsi && theoreticalPsi.length > i) {
            row += `,${theoreticalPsi[i]},${theoreticalDelta[i]}`;
        }
        if (optimizationResults) {
            row += `,${optimizationResults.psi_theoretical[i]},${optimizationResults.delta_theoretical[i]}`;
        }
        
        csvContent += row + '\n';
    }
    
    downloadCSV(csvContent, `psi_delta_data_${new Date().toISOString().slice(0,10)}.csv`);
}

/**
 * Descarga la gráfica de n, k como PNG
 */
function downloadNKGraphPNG() {
    const graphN = document.getElementById('nk-graph-n');
    const graphK = document.getElementById('nk-graph-k');
    ...
    // Descargar gráfica de n
    Plotly.downloadImage(graphN, {
        format: 'png',
        width: 1000,
        height: 500,
    ...
    // Descargar gráfica de k después de un pequeño delay
    setTimeout(() => {
        Plotly.downloadImage(graphK, {
            format: 'png',
            width: 1000,
            height: 500,
...
/**
 * Descarga los datos de n, k como CSV
 */
function downloadNKDataCSV() {
    ...
    downloadCSV(csvContent, `constantes_opticas_nk_${new Date().toISOString().slice(0,10)}.csv`);
}

/**
 * Descarga la gráfica de T-R-A como PNG
 */
function downloadTRAGraphPNG() {
    const graphDiv = document.getElementById('tra-graphs');
    if (!graphDiv) {
        alert('No hay gráfica para descargar');
        return;
    }
    ...
    Plotly.downloadImage(graphDiv, {
        ...
        width: 1200,
        height: 600,
        filename: `espectros_TRA_${new Date().toISOString().slice(0,10)}`
    });
}

/**
 * Descarga los datos de T-R-A como CSV
 */
function downloadTRADataCSV() {
    let traData;
    if (optimizationResults?.tra_spectra) {
        traData = optimizationResults.tra_spectra;
    } else if (window.theoreticalTRASpectra) {
    ...
    if (!traData) {
        alert('No hay datos de T-R-A para descargar');
        return;
    }
    
    const wavelengths = traData.wavelengths || uploadedWavelengths;
    
    let csvContent = 'Wavelength_nm,Transmitancia,Reflectancia,Absorbancia\n';
    ...
    for (let i = 0; i < wavelengths.length; i++) {
        ...
        csvContent += `${wavelengths[i]},${T},${R},${A}\n`;
    }
    
    downloadCSV(csvContent, `espectros_TRA_${new Date().toISOString().slice(0,10)}.csv`);
}
...
// ==========================================
// MODIFICAR showVisualizationTabs EXISTENTE
// ==========================================

/**
 * REEMPLAZA la función showVisualizationTabs existente
 * Esta versión usa el nuevo sistema de pestañas
 */
function showVisualizationTabs(context = 'theoretical') {
    showAdvancedVisualization(context);
}
...
document.addEventListener('DOMContentLoaded', function() {
    // Inicializar sistema de pestañas después de un pequeño delay
    setTimeout(() => {
...
window.initVisualizationTabs = initVisualizationTabs;
window.switchVisualizationPanel = switchVisualizationPanel;
window.showAdvancedVisualization = showAdvancedVisualization;
window.showVisualizationTabs = showVisualizationTabs;
window.downloadPsiDeltaGraphPNG = downloadPsiDeltaGraphPNG;
window.downloadPsiDeltaDataCSV = downloadPsiDeltaDataCSV;
window.downloadNKGraphPNG = downloadNKGraphPNG;
window.downloadNKDataCSV = downloadNKDataCSV;
window.downloadTRAGraphPNG = downloadTRAGraphPNG;
window.downloadTRADataCSV = downloadTRADataCSV;

console.log('✅ Módulo de visualización de pestañas cargado');